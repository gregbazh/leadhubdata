...
export default async function ProductPage({ id }: { id: string }) {
  ...
  const stats = [
    { value: product.leadCount, label: contractor ? "contractor contact records" : "business filing records" },
    ...
    <main>
      <section className="mx-auto grid max-w-6xl gap-10 px-6 py-16 md:grid-cols-[1.4fr_1fr] md:py-24">
        <div>
          ...
          {contractor && <p className="mt-3 text-sm leading-relaxed text-foreground/60">Every record includes a researched phone number, email or contact form. Businesses without a contact method are excluded.</p>}
        ...
        <aside className="self-start rounded-3xl border border-blue/15 bg-blue/[0.025] p-7">
          ...
          <ul className="mt-5 space-y-4 text-sm leading-relaxed text-foreground/65">
            ...
            <li>{contractor ? "Contacts are researched separately and matched to the business. Contact-source links and check dates are included where available; a source check is not mailbox verification." : "Every row has a filing email. Contacts can be an operator or a representative, and inboxes can repeat."}</li>
