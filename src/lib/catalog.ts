...
export const getCatalog = cache(async (): Promise<OneTimeProduct[]> => {
  ...
  const [refreshes, mix, ...counts] = await Promise.all([
    ...
    ...oneTimeProducts.map(p => sql.query(`SELECT count(*)::int AS total,
      ...
      count(*) FILTER (WHERE coalesce(trim(email),'') <> '' OR coalesce(trim(phone),'') <> '' ${p.id === "fl-contractors" ? "OR coalesce(trim(contact_form_url),'') <> ''" : ""})::int AS reachable,
      count(DISTINCT nullif(lower(trim(email)),''))::int AS unique_emails
      ${p.id !== "fl-contractors" ? ", count(DISTINCT nullif(county,''))::int AS counties, max(to_date(application_date,'MM/DD/YYYY'))::text AS latest_application" : ", count(*) FILTER (WHERE coalesce(trim(contact_form_url),'') <> '')::int AS contact_forms"}
      FROM ${p.table}`)),
  ]);
  return oneTimeProducts.map((p, i) => {
    ...
    return {
      ...
      contactFormCount: stat.contact_forms,
